...
class StepByStep(object):
    ...
    def lr_range_test(self, data_loader, end_lr, num_iter=100, step_mode='exp', alpha=0.05, ax=None):
        # Since the test updates the model and optimizer
        # We need to store their initial states to restore them at the end
        ...
        # Build custom function and corresponding scheduler
        lr_fn = StepByStep.make_lr_fn(start_lr, end_lr, num_iter)
        scheduler = LambdaLR(self.optimizer, lr_lambda=lr_fn)

        # Variables to track results and iterations
        tracking = {'loss': [], 'lr': [], }
        iteration = 0

        # If there are more iterations than mini-batches in the data loader
        # we must loop multiple times
        while (iteration < num_iter):
            # This is the typical mini-batch inner loop
            for x_batch, y_batch in data_loader:
                ...
                # Here, track loss (smoothed) and learning rate
